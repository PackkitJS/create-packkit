...
import { PNPM_PIN, YARN_PIN } from '../versions.js';
...
export default {
  ...
  apply(cfg) {
    ...
    const pkg = {
      ...
      // Corepack/pnpm-action-setup read the tool version from here; without it the
      // generated CI fails on its first run. pnpm and modern yarn both need it.
      ...(cfg.packageManager === 'pnpm'
        ? { packageManager: PNPM_PIN }
        : cfg.packageManager === 'yarn'
          ? { packageManager: YARN_PIN }
          : {}),
      scripts: {},
    };
...
function readme(cfg) {
  ...
  const lines = [
    `# ${cfg.name}`,
    '',
    cfg.description || '_A modern package scaffolded with [Packkit](https://packkit-web.pages.dev/)._',
    '',
  ];
  ...
  // First-push friction: a brand-new repo has no lockfile yet, so CI's frozen
  // install fails immediately — and if it was created from the web (no local
  // install), there's been no chance to make one. Say it where someone will
  // actually read it, not just a log. (Shared, package-manager-aware note.)
  lines.push(...ciFirstPushNote(cfg));
